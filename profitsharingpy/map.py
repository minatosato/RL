...
class Map:
    ...
    def step(self):
        ...
        self.episode.append([self.agent.y,self.agent.x,self.action.d])
        self.agent.move(self.action)
        self.state = self.field[self.agent.y][self.agent.x]
        # self.action.update_q_value(self.state)
        if self.state.k == 1:
            for y,x,d in self.episode:
                self.field[y][x].get_action(d).update_q_value(self.state.r,len(self.episode))
